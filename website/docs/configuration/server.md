...
The `server` section configures the ACME HTTP endpoint — the address Hortval listens on and the public URL exposed to ACME clients.
...
|---|---|---|---|
| `url` | — | Yes | Public URL(s) ACME clients use to reach Hortval. Used to build all ACME directory links. |
...
`url` is mandatory. It must match the hostname that ACME clients will use to reach Hortval. Hortval embeds this URL in the ACME directory response and in all object links (orders, authorizations, challenges).
...
If Hortval sits behind a reverse proxy (nginx, Caddy, IIS ARR…), set `remote-ip-header` and `trusted-proxies` to preserve the original client IP in logs and audit records:
