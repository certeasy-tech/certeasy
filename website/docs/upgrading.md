...
### The default working directory moved, and the old one is refused

`%ProgramData%\hortval`, `/var/lib/hortval`, `~/Library/Application Support/hortval`.

If `workdir` is unset **and** the old directory still holds data, startup stops and
offers three ways out: move it, name it explicitly with `workdir:`, or delete it if
it is a leftover. Nothing is moved for you — that directory holds the database, the
node identity, the audit log and the CA key. Setting `workdir` to an absolute path
...
| Behaviour changed | `adcs` connector, `default-timeout` | — | partial sections, rate limits, rotation | command name, `workdir` default, config lookup, licence host, Let's Encrypt key type |
